// Build tasks for the Pinecone CLI project.
//   Modified from those in the gh CLI project,
//   a project by GitHub at https://github.com/cli/cli
//
...
//     - PINECONE_VERSION: determined from source by default
//     - PINECONE_OAUTH_CLIENT_ID
//     - PINECONE_OAUTH_CLIENT_SECRET
...
var tasks = map[string]func(string) error{
	"bin/pinecone": func(exe string) error {
		...
		ldflags = fmt.Sprintf("-X github.com/pinecone-io/cli/internal/build.Version=%s %s", version(), ldflags)
		ldflags = fmt.Sprintf("-X github.com/pinecone-io/cli/internal/build.Date=%s %s", date(), ldflags)
		if oauthSecret := os.Getenv("GH_OAUTH_CLIENT_SECRET"); oauthSecret != "" {
			ldflags = fmt.Sprintf("-X github.com/pinecone-io/cli/internal/authflow.oauthClientSecret=%s %s", oauthSecret, ldflags)
			ldflags = fmt.Sprintf("-X github.com/pinecone-io/cli/internal/authflow.oauthClientID=%s %s", os.Getenv("PINECONE_OAUTH_CLIENT_ID"), ldflags)
		}

		return run("go", "build", "-trimpath", "-ldflags", ldflags, "-o", exe, "./cmd/pinecone")
	},
...
func main() {
	...
	if len(args) < 2 {
		if isWindowsTarget() {
			args = append(args, filepath.Join("bin", "pinecone.exe"))
		} else {
			args = append(args, "bin/pinecone")
		}
	}
...
func version() string {
	if versionEnv := os.Getenv("PINECONE_VERSION"); versionEnv != "" {
		return versionEnv
	}
